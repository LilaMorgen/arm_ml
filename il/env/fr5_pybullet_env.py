...
class FR5_Env(gym.Env):
    ...
    def __init__(self, gui=False):
        ...
        # 创建障碍物
        collisionBlockId = self.p.createCollisionShape(shapeType=p.GEOM_CYLINDER,
                                                       radius=0.03, height=1)
        self.block = self.p.createMultiBody(baseMass=0,
                                            baseCollisionShapeIndex=collisionBlockId,
    ...
    def step(self, action):
        ...
        self_block_contact_points = p.getContactPoints(bodyA=self.fr5, bodyB=self.block)
        ...
        distance = np.sqrt((gripper_centre_pos[0] - self.goalx) ** 2 + (gripper_centre_pos[1] - self.goaly) ** 2 + (
    ...
    def get_observation(self):
        ...
        obs = np.hstack((obs_joint_angles, obs_gripper_centre_pos, obs_target_position),
                                     dtype=np.float32).flatten()

        obs = obs.flatten()
        obs = obs.reshape(1, 12)
